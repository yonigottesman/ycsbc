...
piwiDataPath = '/home-local/erangi/yahoo/YCSB-C/'
piwiDataDir = piwiDataPath+'data_piwi'
...
def deleteFiles():
    ...
    subprocess.call("du -h " + piwiDataDir+ "/", shell=True)
    print("\n", end="", flush=True)
    subprocess.call("ls -l " + piwiDataDir+ "/ | wc -l", shell=True)
    print("deleting data files... ", end="", flush=True)
    subprocess.call("find " + piwiDataDir+ "/ -name '*user*' -delete", shell=True)    
...
def popenAndCall(bench, workload, inits, opers, msize, threads, opt, split, serial):
    ...
    try:
        cmd = './ycsbc -db {0} -P workloads/{1}.spec -threads {2} -recordcount {3} -operationcount {4} -munkBytesCapacity {5} -dbdir {7} {6}' \
             .format(bench, workload, threads, inits, opers, msize, opt, piwiDataPath)
        print('\t\tStarting: ' + cmd)
...
#initsNum = ['0']
#opersNum = ['60000000']
#piwi_options = ['-maxMunks 4000 -writeBufBytesCapacity 524288 -initChunks 5']
...
#initsNum = ['0']
#opersNum = ['60000000']
#piwi_options = ['-maxMunks 32768 -writeBufBytesCapacity 524288 -initChunks 5']
...
#initsNum = ['0']
#opersNum = ['60000000']
#piwi_options = ['-maxMunks 400 -writeBufBytesCapacity 524288 -initChunks 5']
