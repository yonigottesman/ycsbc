...
rocksPath = '/home-local/erangi/yahoo/YCSB-C/'
rocksDataDir = rocksPath+'data_rocksdb'
...
def recordRun(name):
    ...
    try:
        ...
        if name.find('rocks') != -1:
            shutil.copyfile(rocksDataDir+'/LOG', resultsDir+name + '+LOG')
...
def deleteFiles():
    ...
    subprocess.call("du -h "+ rocksDataDir + "/", shell=True)
    print("\n", end="", flush=True)
    subprocess.call("ls -l "+rocksDataDir+"/ | wc -l", shell=True)
    print("deleting data files... ", end="", flush=True)
    subprocess.call("rm -f "+rocksDataDir+"/*", shell=True)
    print("done")
