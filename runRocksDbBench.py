...
rocksPath = '/home-local/erangi/yahoo/YCSB-C/data_piwi/'
rocksdbDataDir = rocksPath+'data_rocksdb'
...
def recordRun(name):
    ...
    try:
        ...
        if name.find('rocks') != -1:
            shutil.copyfile(rocksdbDataDir+'/LOG', resultsDir+name + '+LOG')
...
def deleteFiles():
    ...
    subprocess.call("du -h "+rocksdbDataDir+"/", shell=True)
    print("\n", end="", flush=True)
    subprocess.call("ls -l "+rocksdbDataDir+"/ | wc -l", shell=True)
    print("deleting data files... ", end="", flush=True)
    subprocess.call("rm -f "+rocksdbDataDir+"/*", shell=True)
    print("done")
...
def popenAndCall(bench, workload, inits, opers, msize, threads, opt, dist, serial):
    ...
    try:
        ...
        subprocess.call("tar -C "+rocksdbDataDir+" -xzf "+dbPath+"/savedDb.tar.gz", shell=True)
        ...
        cmd = './ycsbc -db {0} -P workloads/{1}.spec -threads {2} -recordcount {3} -operationcount {4} -munkBytesCapacity {5} -dbdir {7} {6}' \
             .format(bench, workload, threads, inits, opers, msize, opt, rocksPath)
        print('\t\tStarting: ' + cmd)
...
rocks_options = ['-rocksdb_cachesize 1073741824 -use_existing_db 1']
piwi_options = ['']
